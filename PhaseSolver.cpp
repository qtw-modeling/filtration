	...
	const double eta)
{
	...
	double norm = (sumc + (nLO + nHO + nG + 1) * eps * sumc) / sumc;

	for (int i = 0; i <= nLO + nHO + nG; i++)
		this->c[i] = (c[i] + eps * sumc) / norm;
	for (int i = nLO + nHO + nG +1; i < c.N(); i++)
		this->c[i] = c[i] / norm;
...
void PhaseSolver::computeF() {
	...
	double Lx_lam = (nLO + nHO > 0) ? Lx(lam) : 0;
...
void PhaseSolver::computeFx() {
	...
	double Lxx_lam = (nLO + nHO > 0) ? Lxx(lam) : 0;
